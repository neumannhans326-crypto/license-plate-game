...
class MainViewModel(private val database: AppDatabase) : ViewModel() {
    ...
    private val kennzeichenRepository = KennzeichenRepository(database.kennzeichenDao())
    ...
    fun onKennzeichenInput(kennzeichen: String) {
        ...
        viewModelScope.launch {
            ...
            if (existingScan) {
                ...
                _scanResult.value = ScanResult.createAlreadyScanned(playerName, timeStr, cleaned)
                return@launch
            }

            val kennzeichenEntity = database.kennzeichenDao().getByKuerzelSync(cleaned)
            if (kennzeichenEntity == null) {
                _scanResult.value = ScanResult.createNotFound(cleaned)
                return@launch
            }
            ...
            _scanResult.value = ScanResult.createSuccess(kennzeichenEntity, punkte, info)
    ...
    data class ScanResult(
        ...
        companion object {
            fun createSuccess(kennzeichenEntity: KennzeichenEntity, punkte: Int, info: String) = 
                ScanResult(ScanResultType.Success, kennzeichenEntity, punkte, info)
            fun createAlreadyScanned(playerName: String, time: String, kennzeichen: String) = 
                ScanResult(ScanResultType.AlreadyScanned, playerName = playerName, time = time, kennzeichen = kennzeichen)
            fun createNotFound(kennzeichen: String) = 
